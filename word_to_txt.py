"""
Word to Text Conversion Service (LibreOffice-based)
Exports DOC/DOCX to TXT using LibreOffice headless conversion.
"""
...
def word_to_txt(input_path: str, output_path: str) -> Dict:
    try:
        ...
        expected = out_dir / f"{in_path.stem}.txt"

        # 'txt' export is supported by LibreOffice
        cmd = [
            soffice,
            "--headless",
            "--nologo",
        ...
        if not expected.exists():
            alt = list(out_dir.glob(f"{in_path.stem}*.txt"))
            if alt:
                expected = alt[0]
            else:
                return {"success": False, "error": f"Output TXT not found in {out_dir} after conversion."}
