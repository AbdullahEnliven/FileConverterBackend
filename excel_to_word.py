"""
Excel to Word Conversion Service (Best Practical Fidelity)
Pipeline:
  XLS/XLSX -> PDF (LibreOffice) -> DOCX (pdf2docx via pdf_to_word module)

This preserves layout MUCH better than rebuilding tables manually.
"""
...
def excel_to_word(input_path: str, output_path: str) -> Dict:
    try:
        ...
        if proc.returncode != 0:
            return {
                "success": False,
                "error": f"LibreOffice Excel->PDF failed.\nSTDERR: {proc.stderr.strip()}\nSTDOUT: {proc.stdout.strip()}",
            }
